...
function appendLabelsToBody(){
    ...
    for(var i = 0; i < items.length; ++i){
        ...
        html +=     "<div class='label'>" +
                        "<label class='deptHeader'>" + "SIUe - " + items[i]["deptAbbr"] + "</label></br>" +
                        "<label class='itemName'>" + items[i]["itemName"] + "</label>" +
                        "<div class='location'>" +
                            "<span><label class='loc'> " + 
                                items[i]["bldgAbbr"] + items[i]["roomIn"] + "</br>" + items[i]["shelfUnitIn"] + " - " + items[i]["shelfIn"] + 
                            "</label></span></br>" +
                        "</div>" +
                        "<div class='barcode'>" +
                            "<div id='demo" + i + "'/></div>" +
...
function initBarcodes(){
    for(var i = 0; i < items.length; ++i){
        ...
        $("#demo" + i).barcode(
            items[i]["itemIDNumber"], // the id of the item that creates the barcode
            "code39" // we can adjust the created format of the barcode here
        );
    }
}